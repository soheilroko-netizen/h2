...
async function loadConfig() {
  try {
    ...
    (document.getElementById('split_rules') as HTMLTextAreaElement).value = config.split_rules.map(r => r.pattern).join('\n');
  } catch (err) {
...
document.getElementById('btn-delete-profile')?.addEventListener('click', async () => {
  ...
  if (name === 'Default') {
    showMessage('settings-message', 'Cannot delete Default profile', 'error');
    return;
  }
...
document.getElementById('btn-save-split')?.addEventListener('click', async () => {
  const splitRaw = (document.getElementById('split_rules') as HTMLTextAreaElement).value;
  const processRaw = (document.getElementById('process_rules') as HTMLTextAreaElement).value;

  const splitRules = splitRaw.split('\n').map(s => s.trim()).filter(s => s).map(s => ({ pattern: s }));
  const processRules = processRaw.split('\n').map(s => s.trim()).filter(s => s);

  const config = getFormConfig();
  config.split_rules = splitRules;
  // process_rules would need backend support
...
document.getElementById('btn-back-split')?.addEventListener('click', () => {
  window.close();
});
