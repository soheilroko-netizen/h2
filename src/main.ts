...
function updateSplitIndicator(splitMode: string) {
  ...
  if (splitMode === 'custom') tooltipText = 'Split tunnel: Custom (user-defined rules)';
...
splitPresetCards.forEach(card => {
  card.addEventListener('click', async () => {
    ...
    // Parse custom rules only for custom mode; WoW domains hardcoded in backend
    const splitRules: string[] = preset === 'custom' && settingSplitRules.value ? settingSplitRules.value.split('\n').map(r => r.trim()).filter(r => r) : [];
...
btnSaveSettings.addEventListener('click', async () => {
  try {
    ...
    const splitRules: string[] = settingSplitRules.value ? settingSplitRules.value.split('\n').map(r => r.trim()).filter(r => r) : [];
